...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Verificar se o usuário está tentando ver o cardápio público
  ...
  useEffect(() => {
    const channel = supabase.channel('delicias_v1')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'tables' }, (payload) => {
        ...
        if (isLoggedIn && newRec.status === 'occupied' && newRec.current_order) {
  ...
  }, [isLoggedIn, audioEnabled, fetchData]);
  ...
  // 1. VISUALIZAÇÃO DO CLIENTE (Cardápio Público via ?view=menu)
  if (isCustomerView) {
    return (
  ...
  // 2. TELA DE LOGIN (Acesso inicial padrão)
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen bg-brand-cream flex items-center justify-center p-6 relative overflow-hidden font-sans">
        ...
        <div className="absolute bottom-0 left-0 w-80 h-80 bg-brand-green/10 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/2"></div>
        
        <div className="bg-white p-10 md:p-14 rounded-[4rem] w-full max-w-md text-center animate-in zoom-in duration-500 shadow-2xl border-t-8 border-brand-orange relative z-10">
          <div className="bg-brand-orange w-20 h-20 rounded-full mx-auto mb-10 flex items-center justify-center shadow-lg">
            <svg className="w-10 h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 00-2 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
          </div>
          
          <h2 className="text-3xl font-black mb-2 italic uppercase tracking-tighter text-brand-dark">Painel Administrativo</h2>
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-10">Acesso Restrito Ju Marmitas</p>
          
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            <input type="email" placeholder="SEU E-MAIL" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} className="w-full bg-gray-50 border-2 border-gray-100 rounded-3xl px-8 py-5 text-xs font-black uppercase outline-none focus:border-brand-orange transition-all shadow-inner text-center" required />
            <input type="password" placeholder="SUA SENHA" value={loginPass} onChange={e => setLoginPass(e.target.value)} className="w-full bg-gray-50 border-2 border-gray-100 rounded-3xl px-8 py-5 text-xs font-black uppercase outline-none focus:border-brand-orange transition-all shadow-inner text-center" required />
            <button type="submit" disabled={isLoadingLogin} className="w-full bg-brand-dark text-brand-orange font-black py-6 rounded-3xl uppercase text-[11px] tracking-[0.2em] shadow-xl hover:scale-105 active:scale-95 transition-all disabled:opacity-50 mt-4">
              {isLoadingLogin ? 'Autenticando...' : 'Acessar Sistema'}
            </button>
          </form>
        </div>
      </div>
  ...
  // 3. PAINEL ADMINISTRATIVO (Pós-Login)
  return (
    <div className="min-h-screen bg-[#FFF9E5] flex flex-col font-sans relative" onClick={handleUnlockAudio}>
      {activeAlert && (
      ...
      <main className="w-full max-w-6xl mx-auto px-4 sm:px-6 relative z-20 flex-1 pt-8 pb-10">
        <AdminPanel 
