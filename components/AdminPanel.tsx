...
const AdminPanel: React.FC<AdminPanelProps> = ({ 
  ...
  const handleAddCategory = async () => {
    ...
    if (error) alert('Erro ou categoria já existente.');
  ...
  const handleCreateManualOrder = async () => {
    if (!manualCustomer.name || manualOrderItems.length === 0) return alert('Preencha os dados básicos.');
  ...
  const handlePrint = (order: Order) => {
    ...
    const itemsHtml = order.items.map(item => `
      <div style="border-bottom: 1px dashed #ccc; padding: 4px 0;">
        <div style="display:flex;justify-content:space-between;font-weight:bold;"><span>${item.quantity}x ${item.name.toUpperCase()}</span><span>R$ ${(item.price * item.quantity).toFixed(2)}</span></div>
        ${item.observation ? `<div style="font-size: 0.8em; color: #555;">OBS: ${item.observation}</div>` : ''}
      </div>`).join('');
    
    printWindow.document.write(`<html><head><title>Print</title><style>body{font-family:'Courier New',monospace;width:80mm;padding:5mm;font-size:12px;}.center{text-align:center;}.hr{border-bottom:1px solid #000;margin:10px 0;}.flex{display:flex;justify-content:space-between;}</style></head>
      <body onload="window.print();window.close();"><div class="center"><b>${STORE_INFO.name.toUpperCase()}</b></div><div class="hr"></div>
      <div>PEDIDO: #${order.id}</div><div>DATA: ${dateStr}</div><div>TIPO: ${order.orderType === 'delivery' ? 'ENTREGA' : 'RETIRADA'}</div><div class="hr"></div>
      <div>CLIENTE: ${order.customerName.toUpperCase()}</div>${order.address ? `<div style="margin-top:5px;"><b>ENDEREÇO:</b><br/>${order.address}</div>` : ''}<div class="hr"></div>
      ${itemsHtml}<div class="hr"></div><div class="flex"><span>SUBTOTAL:</span><span>R$ ${order.total.toFixed(2)}</span></div>
      ${order.discount ? `<div class="flex"><span>DESCONTO:</span><span>- R$ ${order.discount.toFixed(2)}</span></div>` : ''}
      <div class="flex" style="font-size:1.2em;font-weight:bold;"><span>TOTAL:</span><span>R$ ${order.finalTotal.toFixed(2)}</span></div><div class="hr"></div>
      <div>PAGAMENTO: ${order.paymentMethod.toUpperCase()}</div></body></html>`);
    ...
    <div className="w-full animate-in fade-in duration-500 pb-10">
      ...
      <div className="bg-brand-dark p-6 rounded-[2.5rem] shadow-2xl mb-8 flex flex-col md:flex-row justify-between items-center gap-6 border-b-8 border-brand-orange">
        <div className="flex items-center gap-4">
          <button onClick={onTestSound} className="bg-brand-orange p-3 rounded-2xl text-white shadow-lg active:scale-95"><VolumeIcon size={24}/></button>
          <div><h2 className="text-xl font-black italic text-brand-orange uppercase leading-none">Ju Admin</h2><p className="text-[9px] text-white uppercase font-black tracking-widest mt-1">Gestão Marmitas</p></div>
        </div>
      ...
      <div className="min-h-[60vh]">
        ...
        {activeTab === 'delivery' && (
          <div className="space-y-12">
            <div className="flex justify-between items-center bg-white p-6 rounded-[2rem] shadow-sm border">
               <h3 className="text-xl font-black uppercase italic">Fluxo de Cozinha</h3>
               <button onClick={() => setIsManualOrderModalOpen(true)} className="bg-brand-dark text-brand-orange px-8 py-4 rounded-2xl font-black text-[10px] uppercase shadow-lg hover:scale-105 active:scale-95 transition-all">Lançar Pedido (PDV)</button>
            </div>
        ...
        {activeTab === 'menu' && (
          <div className="bg-white p-10 rounded-[4rem] shadow-xl border-t-8 border-brand-dark">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-10">
              ...
              <select value={selectedCatFilter} onChange={e => setSelectedCatFilter(e.target.value)} className="bg-gray-50 border-2 rounded-2xl px-6 py-4 text-[10px] font-black outline-none">{categories.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}<option value="Todas">Todas</option></select>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {filteredProds.map(item => (
                <div key={item.id} className="bg-gray-50 p-4 rounded-[2.5rem] border-2 border-transparent hover:border-brand-orange/20 shadow-sm flex flex-col">
                  ...
                  <div className="flex-1"><p className="text-[8px] font-black text-brand-green uppercase mb-1">{item.category}</p><h4 className="font-black text-[11px] uppercase truncate">{item.name}</h4><p className="text-[10px] font-black text-brand-orange mt-1">R$ {item.price.toFixed(2)}</p></div>
        ...
        {/* ABA MARKETING (FIDELIDADE E CUPONS RESTORED) */}
        {activeTab === 'marketing' && (
          <div className="space-y-10">
            ...
            <div className="bg-white p-12 rounded-[3rem] shadow-xl border-t-8 border-brand-dark">
              <h3 className="text-2xl font-black italic uppercase mb-10">Ranking Fidelidade</h3>
              <div className="overflow-x-auto no-scrollbar">
                <table className="w-full text-left">
                  <thead className="border-b-2 text-[10px] font-black uppercase text-gray-400"><tr><th className="pb-4">Cliente</th><th className="pb-4">WhatsApp</th><th className="pb-4 text-right">Saldo Acumulado</th></tr></thead>
                  <tbody className="divide-y text-[11px] font-bold">
                    {loyaltyUsers.map(user => (
                      <tr key={user.phone}><td className="py-5 uppercase">{user.name}</td><td className="py-5">{user.phone}</td><td className="py-5 text-right text-brand-orange font-black">R$ {user.accumulated.toFixed(2)}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
          ...
          <div className="max-w-2xl mx-auto bg-white p-12 rounded-[4rem] shadow-xl border-t-8 border-brand-dark">
            ...
            <div className="space-y-8">
               <div className="flex items-center justify-between p-8 bg-gray-50 rounded-[2.5rem] border-2 border-dashed">
                  <div><p className="font-black text-[12px] uppercase">Receber Pedidos Delivery</p><p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Ativa a opção de entrega no cardápio</p></div>
               ...
               <div className="flex items-center justify-between p-8 bg-gray-50 rounded-[2.5rem] border-2 border-dashed">
                  <div><p className="font-black text-[12px] uppercase">Receber Pedidos Retirada</p><p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Ativa a opção de balcão no cardápio</p></div>
      ...
      {isManualOrderModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-brand-dark/95 backdrop-blur-md">
          <div className="bg-white w-full max-w-5xl h-[90vh] rounded-[3rem] p-10 relative flex flex-col shadow-2xl overflow-hidden">
            <button onClick={() => setIsManualOrderModalOpen(false)} className="absolute top-6 right-6 p-4 bg-gray-100 rounded-full active:scale-90"><CloseIcon size={24}/></button>
            <div className="mb-8"><h3 className="text-3xl font-black uppercase italic tracking-tighter text-brand-dark leading-none">Ponto de Venda (PDV)</h3><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Lançamento de pedido manual</p></div>
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-10 overflow-hidden">
               <div className="lg:col-span-7 flex flex-col gap-6 overflow-hidden">
                  <input type="text" value={pdvSearchTerm} onChange={e => setPdvSearchTerm(e.target.value)} placeholder="BUSCAR ITEM..." className="w-full bg-gray-50 border-2 rounded-2xl px-6 py-4 text-xs font-black outline-none focus:border-brand-orange" />
                  <div className="flex-1 overflow-y-auto no-scrollbar space-y-3">
                     {menuItems.filter(p => p.isAvailable && p.name.toLowerCase().includes(pdvSearchTerm.toLowerCase())).map(item => (
                        ...
                        }} className="w-full flex items-center gap-4 bg-white p-4 rounded-[1.5rem] border-2 border-transparent hover:border-brand-orange/30 shadow-sm text-left active:scale-[0.98] transition-all">
                           <img src={item.image} className="w-16 h-16 rounded-xl object-cover shadow-inner" onError={e => e.currentTarget.src='https://placehold.co/100x100'}/>
                           <div className="flex-1"><p className="text-[13px] font-black uppercase text-brand-dark">{item.name}</p><p className="text-[10px] font-black text-brand-orange mt-1">R$ {item.price.toFixed(2)}</p></div>
               ...
               <div className="lg:col-span-5 flex flex-col gap-6 bg-gray-50 p-8 rounded-[3rem] border-2 border-dashed border-gray-200 overflow-hidden shadow-inner">
                  <div className="space-y-4">
                    ...
                    <input value={manualCustomer.phone} onChange={e => setManualCustomer({...manualCustomer, phone: e.target.value})} placeholder="WHATSAPP (OPCIONAL)" className="w-full bg-white border-2 rounded-xl px-5 py-4 text-xs font-black outline-none focus:border-brand-orange shadow-sm" />
                    <div className="flex gap-2">
                       {(['counter', 'delivery'] as const).map(type => (
                         <button key={type} onClick={() => setManualCustomer({...manualCustomer, type})} className={`flex-1 py-4 rounded-xl text-[9px] font-black uppercase border-2 transition-all ${manualCustomer.type === type ? 'bg-brand-dark text-white border-brand-dark shadow-md' : 'bg-white text-gray-400'}`}>{type === 'counter' ? 'Retirada' : 'Entrega'}</button>
                       ))}
                    </div>
                    {manualCustomer.type === 'delivery' && <textarea value={manualCustomer.address} onChange={e => setManualCustomer({...manualCustomer, address: e.target.value})} placeholder="ENDEREÇO COMPLETO..." className="w-full bg-white border-2 rounded-xl px-5 py-4 text-xs font-black h-20 resize-none outline-none shadow-sm animate-in slide-in-from-top duration-300"/>}
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 pt-4 border-t">
                     {manualOrderItems.map(item => (
                        <div key={item.id} className="flex justify-between items-center text-[10px] font-black uppercase bg-white p-3 rounded-xl shadow-sm border">
                           <span className="flex-1"><span className="text-brand-orange mr-2">{item.quantity}x</span>{item.name}</span>
                     ...
                     {manualOrderItems.length === 0 && <p className="text-center py-10 opacity-20 font-black uppercase text-[10px]">Sacola Vazia</p>}
                  </div>
                  <div className="border-t pt-6"><div className="flex justify-between items-end mb-6"><div><p className="text-[9px] font-black uppercase text-gray-400">Total</p><p className="text-4xl font-black italic text-brand-dark leading-none">R$ {manualOrderItems.reduce((a, b) => a + (b.price * b.quantity), 0).toFixed(2)}</p></div></div><button onClick={handleCreateManualOrder} className="w-full bg-brand-dark text-brand-orange py-6 rounded-2xl font-black uppercase text-[11px] shadow-2xl active:scale-95 transition-all">Finalizar Pedido</button></div>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 bg-brand-dark/95 backdrop-blur-md">
           <div className="relative bg-white w-full max-w-2xl h-[80vh] rounded-[2.5rem] p-10 overflow-y-auto flex flex-col border-t-8 border-brand-orange shadow-2xl">
              <div className="flex justify-between items-start mb-6">
                 ...
                 <div className="flex gap-2">
                    <button onClick={() => handlePrint(selectedOrder.currentOrder!)} className="bg-brand-dark text-brand-orange p-3 rounded-full hover:scale-110 shadow-lg"><PrinterIcon size={20}/></button>
              ...
              <div className="flex-1 space-y-6">
                 <div className="bg-gray-50 p-6 rounded-2xl border">
                    <p className="text-[8px] font-black text-gray-400 mb-4 uppercase tracking-widest">Status do Pedido</p>
              ...
              <div className="pt-6 mt-6 border-t flex justify-between items-center"><div className="text-left"><p className="text-[9px] text-gray-400 uppercase font-black">Total</p><p className="text-3xl font-black italic">R$ {selectedOrder.currentOrder?.finalTotal.toFixed(2)}</p></div><button onClick={() => handleCompleteOrder(selectedOrder)} className="bg-brand-green text-white px-10 py-5 rounded-2xl font-black uppercase text-[10px] shadow-xl hover:brightness-110 transition-all">Concluir ✅</button></div>
           </div>
        </div>
      )}

      {/* MODAL CUPOM (RESTORED) */}
      {isCouponModalOpen && (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
           <div className="bg-white w-full max-w-md rounded-[2.5rem] p-10 relative shadow-2xl">
              ...
              <h3 className="text-xl font-black uppercase italic mb-8">Novo Cupom</h3>
              <div className="space-y-5">
                 <input value={editingCoupon.code} onChange={e => setEditingCoupon({...editingCoupon, code: e.target.value})} placeholder="CÓDIGO (EX: JU10)" className="w-full bg-gray-50 border-2 rounded-xl px-5 py-4 text-xs font-black uppercase outline-none focus:border-brand-orange" />
                 ...
                 <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border-2 border-dashed">
                    <span className="text-[10px] font-black uppercase">Cupom Ativo</span>
                    <button onClick={() => setEditingCoupon({...editingCoupon, isActive: !editingCoupon.isActive})} className={`w-12 h-6 rounded-full relative transition-all ${editingCoupon.isActive ? 'bg-brand-green' : 'bg-gray-300'}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${editingCoupon.isActive ? 'left-7' : 'left-1'}`}></div></button>
                 </div>
      ...
      {isProductModalOpen && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-brand-dark/95 backdrop-blur-md">
          <div className="bg-white w-full max-w-lg rounded-[3rem] p-12 relative shadow-2xl overflow-y-auto max-h-[95vh] no-scrollbar">
            <button onClick={() => setIsProductModalOpen(false)} className="absolute top-6 right-6 p-3 bg-gray-100 rounded-full active:scale-90"><CloseIcon size={20}/></button>
            <h3 className="text-2xl font-black italic mb-8 uppercase text-center">{editingProduct?.id ? 'Editar' : 'Nova'} Marmita</h3>
            <form onSubmit={(e) => { e.preventDefault(); onSaveProduct(editingProduct); setIsProductModalOpen(false); }} className="space-y-5">
              ...
              <div className="flex items-center justify-between bg-gray-50 p-6 rounded-2xl border-2 border-dashed"><div><span className="text-[10px] font-black uppercase tracking-widest">Disponível</span></div><button type="button" onClick={() => setEditingProduct({...editingProduct, isAvailable: !editingProduct.isAvailable})} className={`w-14 h-7 rounded-full relative transition-all ${editingProduct?.isAvailable ? 'bg-brand-green' : 'bg-red-400'}`}><div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${editingProduct?.isAvailable ? 'left-8' : 'left-1'}`}></div></button></div>
              <button type="submit" className="w-full bg-brand-dark text-brand-orange py-6 rounded-2xl font-black uppercase text-[11px] shadow-2xl hover:scale-[1.02] transition-all">Salvar Item</button>
            </form>
          </div>
        </div>
      )}
      ...
      {isCategoryModalOpen && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-brand-dark/90 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-10 relative">
            ...
            <div className="flex gap-2 mb-6"><input value={newCatName} onChange={e => setNewCatName(e.target.value)} placeholder="Nova..." className="flex-1 bg-gray-50 border-2 rounded-xl px-4 py-3 text-xs font-black uppercase"/><button onClick={handleAddCategory} className="bg-brand-dark text-brand-orange px-4 py-3 rounded-xl font-black text-[10px] uppercase shadow-lg">Add</button></div>
            <div className="space-y-2 max-h-64 overflow-y-auto no-scrollbar">{categories.map(cat => (<div key={cat.id} className="flex justify-between items-center bg-gray-50 p-4 rounded-xl border shadow-sm"><span className="text-xs font-black uppercase">{cat.name}</span><button onClick={() => handleDeleteCategory(cat.name)} className="text-red-500 hover:scale-110 active:scale-90"><TrashIcon size={18}/></button></div>))}</div>
          </div>
        </div>
      )}
    </div>
