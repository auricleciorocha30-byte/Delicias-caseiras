...
const Cart: React.FC<CartProps> = ({ isOpen, onClose, items, onUpdateQuantity, onRemove, onAdd, onPlaceOrder, storeConfig }) => {
  ...
  const discount = useMemo(() => {
    ...
    return items.reduce((totalDiscount, item) => {
      const validCouponsForItem = appliedCoupons.filter(c => {
        ...
        if (c.scopeType === 'product') {
           return (c.scopeValue || '').trim() === item.id.trim();
        }
        return false;
      });
      ...
      <div className={`fixed right-0 top-0 h-full w-full max-w-md bg-white z-[60] shadow-2xl transition-transform duration-700 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
        {isSuccess ? (
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center font-black animate-in fade-in zoom-in duration-700">
            <div className="w-24 h-24 bg-[#FF7F11]/10 text-[#FF7F11] rounded-full flex items-center justify-center mx-auto mb-10 shadow-2xl">
              <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={4} d="M5 13l4 4L19 7" /></svg>
            </div>
            <h2 className="text-4xl mb-6 italic uppercase tracking-tighter text-[#FF7F11] leading-none">Pedido Enviado!</h2>
            <p className="text-[#666666] uppercase text-[10px] tracking-[0.2em] leading-relaxed">Já recebemos seu pedido e vamos preparar com todo carinho.</p>
            <button onClick={onClose} className="w-full bg-gray-900 text-[#FF7F11] py-7 rounded-[2.5rem] uppercase mt-16 text-[12px] font-black tracking-[0.2em] shadow-2xl hover:scale-105 transition-all">Entendido</button>
          </div>
        ) : (
          <>
              ...
              {items.length > 0 ? (
                <>
                  <div className="bg-gray-50 p-8 rounded-[3rem] border border-gray-100 space-y-8 font-black uppercase shadow-inner">
                    ...
                    <div className="space-y-3">
                      <p className="text-[9px] tracking-[0.3em] text-[#666666] ml-2">Como vai receber?</p>
                      <div className="grid grid-cols-2 gap-2">
                        {storeConfig.counterEnabled && (
                          <button onClick={() => setOrderType('takeaway')} className={`py-4 rounded-2xl text-[9px] border-2 transition-all ${orderType === 'takeaway' ? 'bg-[#FF7F11] text-white border-[#FF7F11] shadow-lg' : 'bg-white text-[#666666] border-gray-100'}`}>Retirada</button>
                        )}
                        {storeConfig.deliveryEnabled && (
                          <button onClick={() => setOrderType('delivery')} className={`py-4 rounded-2xl text-[9px] border-2 transition-all ${orderType === 'delivery' ? 'bg-[#FF7F11] text-white border-[#FF7F11] shadow-lg' : 'bg-white text-[#666666] border-gray-100'}`}>Entrega</button>
                        )}
                      </div>
                    </div>
                    ...
                    {orderType === 'delivery' && (
                      <div className="space-y-3 animate-in slide-in-from-top duration-500">
                        <p className="text-[9px] tracking-[0.3em] text-[#666666] ml-2">Endereço de Entrega</p>
                        <textarea value={address} onChange={(e) => setAddress(e.target.value)} placeholder="RUA, NÚMERO, BAIRRO..." className="w-full bg-white border-2 rounded-2xl px-6 py-5 text-xs outline-none h-28 resize-none focus:border-[#FF7F11] transition-all"/>
                      </div>
                    )}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-3 gap-2">{(['Pix', 'Dinheiro', 'Cartão'] as const).map(method => (
                        <button key={method} onClick={() => setPaymentMethod(method)} className={`py-4 rounded-2xl text-[10px] border-2 transition-all ${paymentMethod === method ? 'bg-[#FF7F11] text-white border-[#FF7F11] shadow-lg' : 'bg-white text-[#666666] border-gray-100'}`}>{method}</button>
                      ))}</div>
                    </div>

                    <div className="flex gap-3 pt-4">
                      <input type="text" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} placeholder="TEM CUPOM?" className="flex-1 bg-white border-2 rounded-2xl px-6 py-5 text-[11px] outline-none uppercase tracking-widest focus:border-[#FF7F11]"/>
                      <button onClick={handleValidateCoupon} className="bg-gray-900 text-[#FF7F11] px-8 py-5 rounded-2xl text-[11px] shadow-xl hover:scale-105 active:scale-95 transition-all">OK</button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-5">
                    {items.map(item => (
                      <div key={item.id} className="flex gap-5 bg-white p-5 rounded-[2.5rem] border items-center relative shadow-sm">
                        <button onClick={() => onRemove(item.id)} className="absolute top-5 right-5 text-red-300 hover:text-red-500 p-2 rounded-xl transition-all z-10"><TrashIcon size={18} /></button>
                        ...
                        <div className="flex-1 font-black">
                          ...
                          <p className="text-[#FF7F11] text-[15px] italic">R$ {item.price.toFixed(2)}</p>
                          <div className="mt-4 flex items-center gap-3 bg-gray-50 p-1.5 rounded-2xl w-fit">
                            <button onClick={() => onUpdateQuantity(item.id, -1)} className="w-8 h-8 rounded-xl bg-white shadow-sm flex items-center justify-center hover:bg-gray-100 transition-all">-</button>
                ...
                <div className="flex flex-col items-center justify-center py-40 opacity-20 grayscale">
                  <p className="font-black uppercase text-[12px] tracking-[0.3em] text-center">Sua sacola está vazia</p>
                </div>
              )}
            </div>
            ...
            {items.length > 0 && (
              <div className="p-10 border-t-8 border-gray-50 bg-white sticky bottom-0 rounded-t-[4rem] z-20 font-black italic shadow-2xl">
                <div className="flex justify-between items-end mb-8">
                  <div>
                    <span className="text-[10px] text-[#666666] not-italic uppercase tracking-[0.25em]">Total a pagar</span>
                ...
                <button 
                  ...
                  className="w-full bg-gray-900 text-[#FF7F11] py-7 rounded-[2.5rem] uppercase text-[13px] font-black tracking-[0.25em] shadow-2xl hover:scale-[1.02] active:scale-95 transition-all"
                >
